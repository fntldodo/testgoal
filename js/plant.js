/* ===================================================
 * 감정 vs 논리 테스트 — 몽실몽실 v2025.3-safe
 * - 전역 오염 없음(IIFE) + 엘리먼트 존재할 때만 실행
 * - assets/plants/* 이미지 1:1 매핑 + 한글명 병기
 * =================================================== */
(function () {
  const PLANT_BASE = "../assets/plants/"; // 필요시 ./ 로만 바꾸면 됨
  ...
  ready(function init() {
    // 필요한 엘리먼트 없으면 아무 것도 하지 않음(충돌 방지)
    ...
    const pillList = list =>
      list.map(t=>`<span class="pill" style="margin-right:6px">${t}</span>`).join('');
        ...
        <div class="result-card mind">
          ...
          <p class="result-desc" style="margin:6px 0 10px;opacity:.9">
            지금의 당신은 <b>${
              ...
              key==='dandelion'?'실용성':
              key==='bamboo'?'유연성':'안정성'
            }</b>이 뚜렷해요. 이 기조를 지키면서 작은 루틴 하나를 더해 리듬을 가볍게 정돈해보면 좋아요.
          </p>
