/* ===================================================
 * 감정 vs 논리 테스트 — 몽실몽실 v2025.3-safe (plant.html 전용)
 * - IIFE(전역 충돌 방지) + DOM 존재 시에만 실행
 * - 결과: 식물 이미지 1:1 매핑 + 한글명 병기
 * - 마음 리마인드 가독성↑, 좌우 분할 그래프(감정← | →논리)
 * =================================================== */
(function () {
  const PLANT_BASE = "../assets/plants/"; // 프로젝트에 맞게 필요 시 "./assets/plants/"로 변경

  const ready = (fn)=>(
    document.readyState!=="loading" ? fn() : document.addEventListener("DOMContentLoaded", fn, {once:true})
  );

  ready(function init(){
    // 필수 엘리먼트 확인(없으면 종료 = 충돌 방지)
