// personality.js v2 — 5지선다(0~4) 통일, 정규화/위트 코멘트 컷 수정
const ITEMS = [
...
function recalc(){
  score.O=score.C=score.E=score.A=score.N=0;
  for(let i=0;i<idx;i++) score[ITEMS[i].k]+=ans[i]??0;
}
  ...
  result.innerHTML = `
    <div class="result-card">
      <div class="result-hero">
        <img src="../assets/brain.png" alt="성격 아이콘" onerror="this.onerror=null; this.src='../assets/brain.png'">
        <div>
...
function normalizeScores(sc){
  const maxPer = 4; const counts={O:4,C:4,E:4,A:4,N:4};
  return {
    O: sc.O/(counts.O*maxPer),
    C: sc.C/(counts.C*maxPer),
    E: sc.E/(counts.E*maxPer),
    A: sc.A/(counts.A*maxPer),
    N: 1 - (sc.N/(counts.N*maxPer)) // 정서안정(역)
  };
}

// 새 최대치(16)에 맞춘 위트 코멘트 컷
function wittyTips(sc){
  const lines=[];
  if(sc.C>=12) lines.push("성실성 만렙! 달력과 메모장이 당신에게 사인 요청🗓️");
  else if(sc.C>=8) lines.push("착착형 생산가. ‘오늘 할 일’을 내일로 미루지 않아요.");
  if(sc.O>=12) lines.push("개방성 팝콘🍿 아이디어가 톡톡 튀어요. 실험 정신 굿!");
  if(sc.E>=12) lines.push("에너지 충만! 하지만 ‘혼자 충전 시간’도 챙기기🔋");
  if(sc.A>=12) lines.push("다정한 공감러 ☕ 대화의 온도를 지키는 사람.");
  if(sc.N<=4)  lines.push("정서 안정! 파도에도 서핑 타는 평정심 🌊");
...
function drawRadar(canvasId, values, keys){
  ...
  keys.forEach((k,i)=>{
    ...
    const label=LABELS[k]; const tw=ctx.measureText(label).width;
