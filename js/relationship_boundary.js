...
(function(){
  ...
  const Q = [
    {pos:'B', neg:'',   q:'그 사람과 있을 때 나의 “싫음/원함”을 분명히 표현할 수 있다.'},
    {pos:'E', neg:'',   q:'감정이 폄하되거나 “예민해”로 치부되지 않는다.'},
    {pos:'T', neg:'',   q:'약속/합의가 바뀌면 서로 확인하고 기록한다.'},
    {pos:'S', neg:'',   q:'서로의 일정/공간/속도를 존중한다.'},

    {pos:'B', neg:'',   q:'부담스러운 요구에는 거절/대안을 제시할 수 있다.'},
    {pos:'E', neg:'',   q:'상대의 실수 지적이 “맞아”로 수용된다.'},
    {pos:'T', neg:'',   q:'메시지·통화 빈도는 합의된 수준을 유지한다.'},
    {pos:'S', neg:'',   q:'내 결정이 상대의 기분으로 자주 뒤집히지 않는다.'},

    {pos:'B', neg:'',   q:'내 과거/관계를 탐문당하는 느낌이 없다.'},
    {pos:'E', neg:'',   q:'가벼운 농담이 모욕/비하로 넘어가지 않는다.'},
    {pos:'T', neg:'',   q:'경제/일정 등 중요한 정보는 숨김없이 공유된다.'},
    {pos:'S', neg:'',   q:'갈등 후 회복 방법(대화 규칙)이 있다.'},

    {pos:'B', neg:'',   q:'내 휴식시간에 대한 침범이 드물다.'},
    {pos:'E', neg:'',   q:'의견충돌에서도 기본적인 다정함이 유지된다.'},
    {pos:'T', neg:'',   q:'중요한 결정 전, 충분한 설명·동의를 구한다.'},
  ];

  const TYPE = {
    SAFE  : {title:'🛟 경계 안전',    emoji:'🛟'},
    SWAY  : {title:'🌬️ 경계 흔들림', emoji:'🌬️'},
    RISK  : {title:'🚧 경계 위험',    emoji:'🚧'},
  };

  const COPY = {
    SAFE:{
      quote:'존중은 관계의 기본 보안장치.',
      desc:'표현-합의-회복 규칙이 작동합니다. 일시적 흔들림만 관리하면 충분해요.',
      summary:['표현 가능','합의 유지','회복 규칙'],
      remind:['휴식·속도 합의 확인','갈등 후 회복 루틴 유지','거절 문장 리허설'],
    },
    SWAY:{
      quote:'지금은 “선 긋기” 리마인드가 필요.',
      desc:'표현/합의가 간헐적으로 무너집니다. 내 속도·공간을 재설정하세요.',
      summary:['의사표현 변동','합의 흔들림','속도 재조율 필요'],
      remind:['요청→합의→기록 순서','메시지 빈도 재합의','거절·대안 문장 준비'],
    },
    RISK:{
      quote:'경계가 무너지면 마음은 지칩니다.',
  ...
  const scorer = ScoreKit.createScorer({ NEG_WEIGHT: 0.4 }); // 부정 극성 낮춤(점검 성격)
  ...
  function finish(){
    ...
    document.getElementById('rEmoji')?.remove(); // 이 페이지는 이모지 헤더 생략 가능(선택)
