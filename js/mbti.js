...
function choose(s){
  ans[idx]=s; const {a}=Q[idx];
  if(a==='E') score.E+=s; if(a==='I') score.I+=s;
  if(a==='S') score.S+=s; if(a==='N') score.N+=s;
  if(a==='T') score.T+=s; if(a==='F') score.F+=s;
  if(a==='J') score.J+=s; if(a==='P') score.P+=s;
  next();
}
...
function recalcTo(end){
  score.E=score.I=score.S=score.N=score.T=score.F=score.J=score.P=0;
  for(let i=0;i<end;i++){
    const s=ans[i]??0; const {a}=Q[i];
    if(a==='E') score.E += s; if(a==='I') score.I += s;
    if(a==='S') score.S += s; if(a==='N') score.N += s;
    if(a==='T') score.T += s; if(a==='F') score.F += s;
    if(a==='J') score.J += s; if(a==='P') score.P += s;
...
function meter(L, Lv, R, Rv){
  const lPct = Math.round(Lv/9*100);
  const rPct = Math.round(Rv/9*100);
