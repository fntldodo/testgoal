/* ===================================================
 * 자립 지수 체크 — 몽실몽실 v2025.2.3 (마음 리마인드)
 * ---------------------------------------------------
 * - 기능 보존: 문항/산식/분류 동일, UI만 정돈
 * - 하이브리드 키 DR/ER/DE 고정
 * - 결과 카드: ①설명(2~3줄) ②핵심 키워드(옅은 칩) ③리마인드(글머리표) ④미터
 * - 미터 우측 라벨: 루틴→유연함 / 결정→선명함 / 평온→적정 (+ %는 옅게)
 * =================================================== */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function classify(){
    ...
    if(Math.abs(v1-v2)<0.10){
      const pair=[k1,k2].sort().join(''); // 'DR','ER','DE'
  ...
  function finish(){
    try{
      ...
      // 핵심 키워드(옅은 칩, 3개)
      const keyChips = info.keywords.map(t=>`<span class="chip-soft">${t}</span>`).join('');

      // 리마인드(글머리표 2~3줄)
      const remindList = `
        <ul class="remind-list">
          ${info.remind.map(r=>`<li>${r}</li>`).join('')}
        </ul>
      `;
      ...
      box.innerHTML = `
        <div class="result-card">
          ...
          <div class="keyline">${keyChips}</div>
